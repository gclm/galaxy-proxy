...
pub async fn proxy(
    ...
    // 选择渠道
    let selection = match state.select_channel(model, EndpointType::OpenAiEmbedding, session_hash.as_deref()).await {
        Ok(s) => s,
        Err(e) => {
    ...
    // 构建 URL
    let url = format!("{}{}", selection.endpoint.base_url, EndpointType::OpenAiEmbedding.path());
